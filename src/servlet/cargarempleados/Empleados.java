package servlet.cargarempleados;
...
public class Empleados extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try 
			...
			rs = st.executeQuery("select first_name, last_name, salary, employee_id from employees where department_id = " + department_id);
			...
			while (rs.next())
			{
				...
				out.println("<salario>"+ rs.getString(3)+"</salario>");
				out.println("<id>"+ rs.getInt(4)+"</id>");
				out.println("</empleado>");
			}
			out.println("</empleados>");
